...
describe("width creation with and without array", () => {
    ...
    test("latin-1 latin extended array creation", async () => {
        ...
        expect(
            measure.measure(
                "ö",
                measure.load("latin-1-supplement-10px-sans-serif.json")
            )
        ).toBe("5.56");
        expect(
            measure.measure(
                "Āā",
                measure.load("latin-extended-a-10px-sans-serif.json")
            )
        ).toBe("12.23");
    });
    ...
    test("basic latin creation", async () => {
        ...
        expect(
            measure.measure(
                "hello",
                measure.load("basic-latin-10px-sans-serif.json")
            )
        ).toBe("21.12");
    });
    ...
    test("basic latin creation with specific font", async () => {
        ...
        expect(
            measure.measure(
                "hello",
                measure.load("basic-latin-10px-ubuntu-mono.json")
            )
        ).toBe("20");
    });
});
